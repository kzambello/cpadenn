...
def CPadeAF_calibration(deg_num, deg_den, crelu, safe=False):
    ...
    x = PadeNN.Layers.CPadeAF(deg_num=deg_num, deg_den=deg_den, lreg=1.0e-3, imean=0.0, istddev=1.0e-4, safe=safe)(x)
    x = PadeNN.Layers.CSplitReIm()(x)

    outputs = x

    modelAF = tf.keras.Model(inputs=inputs, outputs=outputs)
    optadam = tf.keras.optimizers.Adam(learning_rate=1.0e-3)

    modelAF.compile(optimizer = optadam, loss='mean_squared_error')
    ...
    print("\nCalibrating (1)...\n")
    modelAF.fit(Zg, crelu(Zg), epochs=100, batch_size=2048)

    print("\nCalibrating (2)...\n")
    modelAF.fit(Zg, crelu(Zg), epochs=100, batch_size=2048)
    
    alphas = modelAF.get_weights()
...
def GetAlphas(n):
    if n == 4:
        alphas = [ np.array([0.55991364, 0.14828739, 0.01649525, 0.00223316, 0.5429329 ], dtype=np.float32),
                   np.array([ 0.11221588,  0.00310184,  0.00828541, -0.0009095 ], dtype=np.float32),
                   np.array([-6.2417763e-04, -6.3668203e-04,  8.2182858e-05,  1.6188879e-04, -5.9690198e-04], dtype=np.float32),
                   np.array([-1.0941960e-03,  2.2226348e-05, -2.5839164e-04, -1.7531947e-04], dtype=np.float32) ]
        return alphas
