...
class CDense(tf.keras.layers.Layer):
    ...
    def build(self, input_shape):            
        ...
        initializer = tf.keras.initializers.TruncatedNormal(mean=self.imean, stddev=self.istddev)
        regularizer = tf.keras.regularizers.l2(self.lreg)
        
        self.w_r = self.add_weight(shape=(feature_dim, self.units), initializer=initializer, regularizer=regularizer, trainable=True)
        self.b_r = self.add_weight(shape=(self.units,), initializer=initializer, regularizer=regularizer, trainable=True)

        self.w_i = self.add_weight(shape=(feature_dim, self.units), initializer=initializer, regularizer=regularizer, trainable=True)
        self.b_i = self.add_weight(shape=(self.units,), initializer=initializer, regularizer=regularizer, trainable=True)
    ...
    def call(self, inputs):
        ...
        inputs_c = tf.expand_dims(inputs_c, axis=1) # shape becomes (batch_size, 1, feature_dim)
...
class CFugacityCoV(tf.keras.layers.Layer):
    ...
    def call(self, inputs):
        ...
        res = tf.exp(inputs_c)
        res = tf.reshape(res, (-1, 1)) # reshape output from (batch_size,) to  (batch_size, 1)

        return res


class CPadeAF(tf.keras.layers.Layer):
    def __init__(self, deg_num=3, deg_den=3, lreg=1.0e-4, alphas=None, imean=0.0, istddev=1.0, safe=False):
        super().__init__()
        ...
        self.coeffs_num_r = self.add_weight(shape=(deg_num+1,), initializer=initializer, regularizer=regularizer, trainable=True)
        self.coeffs_den_r = self.add_weight(shape=(deg_den,), initializer=initializer, regularizer=regularizer, trainable=True)
        self.coeffs_num_i = self.add_weight(shape=(deg_num+1,), initializer=initializer, regularizer=regularizer, trainable=True)
        self.coeffs_den_i = self.add_weight(shape=(deg_den,), initializer=initializer, regularizer=regularizer, trainable=True)
    ...
    def call(self, inputs):
        ...
        numerator = tf.add_n([coeffs_num_c[i] * tf.pow(inputs_c, i + 1) for i in range(self.deg_num)])
        numerator = tf.add(numerator, coeffs_num_c[self.deg_num])

        denominator = tf.add_n([coeffs_den_c[i] * tf.pow(inputs_c, i + 1) for i in range(self.deg_den)])
...
class CDropout(tf.keras.layers.Layer):
    ...
    def call(self, inputs, training=None):
        ...
        return tf.complex(self.dropout(inputs_r, training=training), self.dropout(inputs_i, training=training))
